...
def build_catalog() -> dict:
    ...
    if round2.get("count") != 53 or len(round2.get("items", [])) != 53:
        raise ValueError("round2 catalog must contain 53 records")
    items.extend(round2["items"])
    if len(items) != 138 or len({x["no"] for x in items}) != 138 or len({x["name"] for x in items}) != 138:
        raise ValueError("catalog must contain 138 unique records")
    return {
        "title": "AKECHI PORT v2｜138外付け能力図鑑",
        "version": "0.2.0", "generated_at": "2026-09-23",
        "source": "Notion｜プラグイン大賞 同級者編 001〜085 + 第2回 新規Plugin増設編 001〜053",
        "count": 138,
...
def render(catalog: dict) -> str:
  ...
  <div class="cards">{cards}</div>
    ...
    return (TEMPLATE
            .replace("{{CHIPS}}", chips)
            .replace("{{SECTIONS}}", "\n".join(sections))
            .replace("{{TOTAL}}", str(catalog["count"]))
            .replace("{{RANGE}}", f"001–{catalog['count']:03d}"))
...
<header class="site-header"><a class="back" href="/">← ひでろっく本館</a><p class="eyebrow">PLUGIN ATLAS / {{RANGE}}</p><h1>AKECHI PORT</h1><p class="lead">{{TOTAL}}個の外部AI能力を、普通の道具と機械のカラクリの両方から眺める図鑑。入力と出力を見て、次につなぐPluginを考える。</p></header>
<main id="catalog">
  <section class="tools" aria-label="検索とジャンル移動">
    ...
    <nav class="genre-nav" aria-label="10ジャンル"><a class="genre-chip" href="#catalog" data-filter="all" aria-current="true">全部 <small>{{TOTAL}}</small></a>{{CHIPS}}</nav>
    <div class="result-line"><span id="result" aria-live="polite">{{TOTAL}}件</span><span>カードを開くと副音声と接続が見えます</span></div>
  </section>
  <div id="empty" class="empty">一致するPluginがありません。検索語かジャンルを変えてください。</div>
  {{SECTIONS}}
</main>
<footer class="site-footer"><p>副音声は理解用の機能モデル。具体例と接続は利用案で、連携を実証した記録とは別です。</p><p>Statusは証拠の強さを示す記録：PROVEN＝実機成功／PARTIAL＝一部実証／BLOCKED＝対象・認証などで実行停止／DEFINED＝現在のTool定義を確認済みだが未実行／HOLD＝保留／UNKNOWN＝照合記録なし。旧85件と第2回53件では観測日が異なるため、各カードのEvidence noteを優先してください。</p><p>正本：ヒデロック発明OS©️ Notion「プラグイン大賞 同級者編 001〜085」／表示データ：<a href="catalog-v2.json">catalog-v2.json</a></p></footer>
<script>
